...
gui = tk.Tk()
gui.title("Aftermath Setup translation by Darkiyus")
...
start_button = tk.Button(gui, text = "Nach der Installation ausführen", command = start_func)
...
def boom(sup:bool=False):
    ...
    if name != 'nt':
        tk.Label(gui, text="Fehler. Aftermath läuft nur unter Windows.", fg="white", bg="black").place(x=xcenter(300), y=100, width=300, height=20)

    elif not exists(startup_path):
        tk.Label(gui, text="Fehler. Der Startpfad ist ungültig.",
                 fg="white", bg="black").place(x=xcenter(320), y=100, width=320, height=20)

    elif not exists(downloads_path):
        tk.Label(gui, text="Fehler. Der Pfad zu den Downloads ist ungültig.",
        ...
        if not sup:
            ...
            ttk.Label(gui, text="Aftermath wurde erfolgreich installiert!", foreground="white", background="black").place(x=xcenter(200), y=150, width=200, height=20)
        else:
            remove(startup_path + "aftermath.pyw")
            ttk.Label(gui, text="Aftermath wurde erfolgreich deinstalliert!", foreground="white", background="black").place(x=xcenter(220), y=150, width=220, height=20)
    tk.Button(gui, text="Verlassen", command=exit, fg="white", bg="black").place(x=xcenter(100), y=60, width=100, height=30)
...
uninstall_button = tk.Button(gui, text="Deinstaller", command=sup, bg="black", fg="white")
...
webhook_button = tk.Button(gui, text = "Webhook für Logs wählen", fg="black", bg="white", command=webhook_choose)
